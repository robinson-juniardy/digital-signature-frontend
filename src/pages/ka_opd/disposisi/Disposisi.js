...
const OPDDisposisi = ({ open, setOpen, filename, rows }) => {
  ...
  const handleProses = async () => {
    await API.post('/api/suratmasuk/proses', {
      status: currentUser.role === 'ka_opd' ? 1 : 4,
  ...
  const handleSelesai = () => {
    API.post('/api/suratmasuk/proses', {
      status: currentUser.role === 'ka_opd' ? 3 : 5,
      id_surat: rows.id_surat,
      created_by: currentUser.nip,
    })
      .then((response) => {
  ...
  const handleUpdateStatus = () => {
    API.post('/api/suratmasuk/proses', {
      ...
      .then((response) => {
        if (response.data.status === 1) {
          enqueueSnackbar('Status Dokumen Berhasil Di Update', {
            variant: 'success',
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: 'bottom',
          ...
          console.log(response.data.message);
          enqueueSnackbar('Status Dokumen Gagal Di Update', {
            variant: 'error',
            autoHideDuration: 3000,
            anchorOrigin: {
              vertical: 'bottom',
  ...
  const handleDisposisi = () => {
    API.post('api/suratmasuk/disposisi', {
      ...
      status: currentUser.disposision_level - 1,
